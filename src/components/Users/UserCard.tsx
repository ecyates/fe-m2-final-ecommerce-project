        ...
        {currUser&&<Card className='userCard' style={{ width: '18rem' }}>
            ...
            <div className='p-2'>
                ...
                <Row className='text-center mt-3'>
                    <Col><Button className='btn btn-primary' onClick={()=>navigate(`/edit-user/${currUser.id}#form`)}>Edit</Button></Col>
                    {/* Cannot delete the current user. */}
                    {currUser.id!==user?.uid?
                        <Col><Button className='btn btn-secondary' onClick={() => deleteUser(currUser.id)}>Delete</Button></Col>:
                        <Col><Button className='btn btn-secondary' onClick={signOut}>Sign Out</Button></Col>
                    }
                </Row>
            </div>
            ): (
            <Card.Text className='text-center'>
                ...
                <Col><Button className='btn btn-primary' onClick={()=>navigate(`/edit-user/${currUser.id}#form`)}>Add Now</Button></Col>
                {currUser.id!==user?.uid&&<Col><Button className='btn btn-secondary' onClick={() => deleteUser(currUser.id)}>Delete</Button></Col>}
                </Row>
            </Card.Text>
            )}
            </Card.Body>
